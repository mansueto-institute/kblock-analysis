...
(scatter_by_country_4way_1 <- ggplot() +
    geom_point(data = scatter_by_country_4way %>% filter(landscan_population_un >= 10000), aes(x = reorder(country_name, (agg_sum_landscan_population_un_group_by_country)), y = log10(landscan_population_un), 
                                                                                               #fill = class_urban_hierarchy, color = class_urban_hierarchy, size = k_complexity_weighted_landscan_un), alpha = .75) +
...
(scatter_by_country_4way_2 <- ggplot() +
    geom_point(data = scatter_by_country_4way, aes(x = reorder(country_name, (k_reweight)), y = k_complexity_weighted_landscan_un, 
                                                   #fill = class_urban_hierarchy, color =  class_urban_hierarchy, size = landscan_population_un), alpha = .75) +
...
(scatter_by_country_4way_3 <- ggplot() +
    geom_point(data = scatter_by_country_4way %>% filter(agg_sum_landscan_population_un_group_by_country >= 1000000, landscan_population_un >= 10000), 
               aes(x = log10(k_complexity_weighted_landscan_un), y = log10(landscan_population_un), 
                   #fill = class_urban_hierarchy, color = class_urban_hierarchy, size = landscan_population_un ), alpha = .7) +
    ...
    #scale_size(range = c(1, 20), breaks = c(10000,100000,1000000,10000000,40000000,80000000), labels = c('10K','100K','1M','10M','40M','80M')) +
...
# scatter_by_country_4way_1 + scatter_by_country_4way_2 + plot_layout(guides = 'collect')
...
(histogram_africa <- ggplot() +
    geom_bar(data = histogram_africa_data, aes(y = group_val, x = landscan_population_un, 
                                               #fill = class_urban_hierarchy), color = 'white', linewidth = .3, stat="identity") +
                                               fill = class_urban_paper), color = 'white', linewidth = .3, stat="identity") + 
    coord_flip() + 
    labs(y= 'Block complexity', x = 'Population', fill = '', color =  '', subtitle = '') + 
    #geom_text(data = histogram_africa_data %>% filter(class_urban_hierarchy %in% c('2 - Peripheral urban', '4 - Non-urban')),
    ...
    #geom_text(data = histogram_africa_data %>% filter(class_urban_hierarchy %in% c('1 - Core urban','3 - Peri-urban') ),
    ...
    scale_fill_manual(values = c('#08519c','#9ecae1','#006d2c','#a1d99b')) +
    #scale_fill_manual(values = c('#0065AF','#00D5F2','#00BA62','#92F58C')) +
...
area4_pie_data <- histogram_africa_data %>% 
  #group_by(class_urban_hierarchy) %>%
  ...
  # mutate(class_urban_hierarchy = factor(class_urban_hierarchy, levels = c( "1 - Core urban","2 - Peripheral urban","3 - Peri-urban","4 - Non-urban"))) %>%
  # arrange(factor(class_urban_hierarchy, levels = rev(c( "1 - Core urban","2 - Peripheral urban","3 - Peri-urban","4 - Non-urban")))) %>%
  mutate(class_urban_paper = factor(class_urban_paper, levels = c( "1 - Urban","2 - Secondary urban","3 - Peri-urban","4 - Rural"))) %>%
...
(area4_pie <- ggplot(data = area4_pie_data, 
    ...
    geom_text(data = area4_pie_data %>% 
                #filter(class_urban_hierarchy %in% c('2 - Peripheral urban', '4 - Non-urban')), 
...
(k_pie <- ggplot(data = k_pie_data, 
    ...
    #geom_text(aes(x = 1.2, y = pos_id_val, label =  ifelse(share >= .01, group_val, '' )), color = "#333333", size=6, fontface = 'bold') +
    scale_fill_manual(values = c(grey2,colorhexes), name = 'Block complexity') + 
    theme_void() + 
    theme(
...
(histogram_africa_pies <- histogram_africa + 
  ...
  #plot_layout(tag_level = 'new') + plot_annotation(tag_levels = c('A')) &
    plot_layout() + plot_annotation(tag_levels = list(c('A','B','C'))) & 
  theme(plot.tag = element_text(size = 13)))

# ggsave(plot = histogram_africa, filename = paste0(wd_output,'/viz/histogram_africa.pdf'), width = 12.4, height = 5.64)
...
histogram_buildings <- histogram_buildings %>%
  ...
  mutate(bin_label = case_when(category == 'bldg_area_count_bin_01_0.50__log10_3.2' ~ '< 5.6', 
                               ...
                               category %in% c('bldg_area_count_bin_11_3.00__log10_1000','bldg_area_count_bin_12_3.25__log10_1778.3',
                                 ...
                                 # category == 'bldg_area_count_bin_11_3.00__log10_1000' ~ '1K to\n1.78K', 
                                 # category == 'bldg_area_count_bin_12_3.25__log10_1778.3' ~ '1.78K to\n3.16K', 
                                 # category == 'bldg_area_count_bin_13_3.50__log10_3162.3' ~ '3.16K to\n5.62K', 
                                 # category == 'bldg_area_count_bin_14_3.75__log10_5623.4' ~ '5.62K to\n10K', 
                                 # category == 'bldg_area_count_bin_15_4.00__log10_10000' ~ '>= 10K'
                               ) %>%
  mutate(bin_label = factor(bin_label, levels = c('< 5.6', '5.6 to\n10', '10 to\n17', '17 to\n31\n(parking\nspace)', '31 to\n56', '56 to\n100', '100 to\n177', '177 to\n316', '316 to\n562\n(basketball\ncourt)', '562 to\n1K', '> 1K'))) %>%
  #group_by(bin_label, class_urban_hierarchy) %>%
...
(bldgs_density <- ggplot(data = histogram_buildings, 
    ...
    #geom_line(linewidth = 1, aes( color = class_urban_hierarchy)) +
    scale_x_discrete(expand = c(0.05,0.05)) +
    scale_fill_manual(values = c('#08519c','#9ecae1','#006d2c','#a1d99b')) +
    scale_color_manual(values = c('#08519c','#9ecae1','#006d2c','#a1d99b')) +
    labs(x = expression("Building footprint area, m"^2) ,
...
urban_bars_data <- generate_crosstabs(data = df_combined_prep %>% 
                                        #filter(class_urban_hierarchy %in% c('1 - Core urban')),
...
scatter_sd
# scatter_sd <- ggplot() + 
#   geom_point(data = agglos_sd, 
#              aes(x = weighted_mean, y = weighted_sd, size = area_total), alpha = .4) +
#   geom_text_repel(data = agglos_sd, seed = 1, segment.curvature = -0.1, point.padding = 0, box.padding = 0.4, max.iter = 30000, segment.square  = FALSE, segment.inflect = FALSE, min.segment.length = .2, max.overlaps = Inf, force = .5, force_pull = 10, 
#                   aes(x = weighted_mean, y = weighted_sd, label = agglosname), size = 3, vjust =.5, color = '#333333', fontface='bold') +
#   scale_size_continuous(range = c(1,10), labels = label_comma(accuracy = 1L, scale =  0.000001, suffix = "M") ) +
#   theme_classic() +
#   labs(y = 'Weighted Standard Deviation', x = 'Population-weighted Block Complexity', size = 'Population')
...
(africa_zoom <- ggplot() +
    ...
    # geom_sf(data = data.frame(place=c('zmb-offset'), longitude=c(-4.682159), latitude=c(-15.016642)) %>%
    #           st_as_sf(., coords = c("longitude", "latitude"), crs = 4326, agr = "constant") %>%
    #           st_transform(3857) %>%
    #           st_buffer(., 1800000) %>% 
    #           st_transform(4326), fill = '#ffffff', alpha = 1, color = '#3288bc', linewidth = 1.5) +
    theme_void() + inset_element(zoom_inset, left = -.5, bottom = .12, right = 1, top = .55, align_to = 'full')
  )

# target_blocks = area_data %>% st_make_valid() %>%
#   st_join(x = ., y = sites %>% st_make_valid() %>% filter(place == place_var), left = FALSE)
...
df_combined_prep %>% group_by(country_code, country_name) %>%
  tally() %>% print(n = 100)
buildings <- arrow::open_dataset(paste0('/Users/nm/Desktop/Projects/work/mnp-analysis/big-data.nosync/inputs/buildings_polygons_ZMB.parquet')) %>%
  filter(gadm_code %in% unique(area_data_framed$gadm_code))%>% 
  read_sf_dataset() %>%
  st_set_crs(4326) %>%
  st_make_valid() %>%
  st_intersection(., sites %>% filter(place == place_var))
  #st_filter(., sites %>% st_make_valid(), .predicate = st_intersects)

streets <- arrow::open_dataset(paste0('/Users/nm/Desktop/Projects/work/mnp-analysis/big-data.nosync/inputs/streets_ZMB.parquet')) %>%
  read_sf_dataset() %>%
  st_set_crs(4326) %>%
  st_make_valid() %>%
  st_intersection(., sites %>% filter(place == place_var))
